  ...
  try {
    ...
    for (const schedule of schedules.rows) {
      ...
      if (schedule.contact_type === 'notary') {
        ...
        const result = await db.execute(sql`
          SELECT id, business_name as first_name, '' as last_name, email,
                 license_expiration as expire_date, license_type as surety_company,
                 city, county
          FROM auto_dealers
          WHERE email != '' AND email IS NOT NULL
          AND email NOT IN (SELECT email FROM unsubscribes)
          AND email NOT IN (
            ...
            WHERE status = 'sent' AND is_auto = true AND drip_id = ${schedule.id}
          )
      ...
      for (const c of contacts) {
        ...
        try {
          ...
          `);
          // Record in contact-type-specific sends table for UI history + SENT badges
          if (schedule.contact_type === 'notary') {
            await db.execute(sql`
